...
async function processDownloadJob(data) {
    const { id, platform, quality, format } = data;
    ...
    // Stage 2: media processing placeholder
    await new Promise((r) => setTimeout(r, 350));
    await markJob(id, { progress: 65, title: `Processing ${quality || "best"} ${format || "mp4"}` });

    // Stage 3: storage upload placeholder key
    const storageKey = `jobs/${id}/video.${format || "mp4"}`;
